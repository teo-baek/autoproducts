...
# 자가가입 허용 역할 — admin/wholesaler 는 관리자가 직접 온보딩(권한 상승 방지)
_SELF_REGISTER_ROLES = {"retail_seller", "agency"}
...
def register_account(repo, req) -> dict:
    ...
    - 자가가입은 retail_seller/agency 만(admin·wholesaler 거부 — 권한 상승 차단).
    ...
    if role not in _SELF_REGISTER_ROLES:
        raise RegisterError("자가가입은 retail_seller/agency 만 허용됩니다")
